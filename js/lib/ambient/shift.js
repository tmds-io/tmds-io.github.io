...
const yOff = 0.0015;
const zOff = 0.0015;
const backgroundColor = 'hsla(0,0%,50%,1)';
...
function initCircle(i) {
    let x, y, z, n, t, speed, vx, vy, life, ttl, radius, hue;

    x = rand(canvas.a.width);
    y = rand(canvas.a.height);
    z = rand(255);
    n = simplex.noise3D(x * xOff, y * yOff, z * zOff);
    t = rand(TAU);
...
function drawCircle(x, y, life, ttl, radius, hue) {
    ctx.a.save();
    ctx.a.fillStyle = `hsla(0,0%,${Math.round(hue)}%,${fadeInOut(life, ttl)})`;
    ctx.a.beginPath();
